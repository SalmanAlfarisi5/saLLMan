...
reasoning + code". SFT teaches that distribution. Three new files:
[finetune_data_prep.py](finetune_data_prep.py),
[finetune.py](finetune.py),
[verify_finetune_data.py](verify_finetune_data.py).

### [finetune_data_prep.py](finetune_data_prep.py)

Builds the SFT corpus from one verified source:
**`open-r1/codeforces-cots`, config `solutions_py_decontaminated`,
split `train`** — Codeforces problems with both DeepSeek-R1 chain-of-
thought traces and a separate human-written editorial.

Three substantive choices, all forced by inspection of the dataset
before coding:

| Choice | What we do | What we don't do |
|---|---|---|
| **Reasoning source** | `row["editorial"]` (concise, human-written) | NOT the R1 `<think>...</think>` block (median ~15 k tokens) |
| **Problem source** | `description + "\n\n" + input_format + "\n\n" + output_format` | NOT `messages[0].content` (includes a flavor-narrative wrapper that bloats most rows past 2048) |
| **Code source** | First ```` ```python ```` block AFTER `</think>` in `messages[1].content` (with `row["generation"]` as fallback) | `</think>` is used **only as a positional marker** for the code search; we extract nothing between the `<think>` tags |

`finish_reason` is **counted but not filtered** — editorial gives a
self-contained reasoning trace, so a length-truncated R1 generation
with an intact `\`\`\`python` block is usable.

#### Output format (tagged sections — no new special tokens needed)

```
<problem>
{description + input_format + output_format, joined with blank lines}
</problem>
...
`</reasoning>` and the `<code>...</code><eos>` block — so the model
learns to emit the boundary markers and stop cleanly on EOS.

#### Why tokenize prompt and response separately

Building the loss mask by hand is much safer than searching for a
delimiter token afterward. BPE may merge `<reasoning>` differently than
`</reasoning>` depending on neighbouring whitespace; separate encodes
guarantee a bit-exact boundary in id space. The layout is:

```
input_ids = [BOS] + prompt_ids + response_ids + [EOS]
loss_mask = [0]   + [0]*L_p    + [1]*L_r      + [1]   ← predict EOS too
```

#### ExtractStats — owned-counter pattern

The extractor takes a caller-owned `ExtractStats` dataclass and mutates
it in place. The caller prints the summary from a `try/finally` block,
so counts survive any mid-loop exception or `^C`. Two partitions of
`n_rows_seen` are tracked and **asserted at print time**:

1. **Outcome partition** — every row ends in exactly one of: `n_yielded`,
   `skip_no_editorial`, `skip_no_description`, `skip_no_input_format`,
   `skip_no_output_format`, `skip_malformed_msgs`, `skip_no_think_close`,
   `skip_no_py_fence`, `skip_unclosed_fence`, `skip_empty_code`.
2. **finish_reason partition** — every row contributes to exactly one of
   `n_finish_stop`, `n_finish_length`, `n_finish_other`. Informational
   (NOT a skip filter).

A future refactor that adds a skip branch without a counter trips the
outcome assertion; a future schema shift with a new `finish_reason`
value trips the finish_reason assertion. No silent mass-skips possible.

#### Length filtering

Examples whose total token count exceeds `--max-len` (default **2048**)
are dropped rather than truncated. Verified length stats on the
`codeforces-cots` source with the trimmed-problem + editorial form:
...
Actual extractor yield (from [finetune_data_v2/meta.json](finetune_data_v2/meta.json)):
- 8 133 rows seen → 4 504 yielded by the extractor → **3 704 kept** after
  the length filter (3 519 train / 185 val at `val_ratio=0.05`).
- Biggest skip bucket: `skip_no_editorial = 3 228` (not all rows have an
  editorial). All other skip buckets are small (< 200 each).
- `n_finish_length = 362` were rescued by dropping the `finish_reason`
  filter — those are the rows the old strict-stop version would have
  thrown away.
...
Standalone (no torch, no GPU). Confirms the prep-script output before
training. Two invariants over every record:

1. **Length match** — `len(input_ids) == len(loss_mask)`.
2. **Single 0→1 flip** — `loss_mask` is `[0]*p + [1]*r`. Any `0` after a
   `1`, or any value outside `{0,1}`, is flagged with the offending
   index and a context window.

For the first `--preview` (default 3) records, prints both:

- The **full** decoded `input_ids` — everything the model sees.
- The **mask==1 only** decode — what loss is actually computed on.

The second print must contain `</reasoning>\n<code>\n...\n</code>` and
must NOT contain `<problem>` or the opening `<reasoning>` tag. Eyeball
the previews before kicking off training; if the masked region leaks
prompt content, the boundary is off by tokens.

#### Run

```bash
python verify_finetune_data.py
python verify_finetune_data.py --preview 5
```
...
Loop structure mirrors `pretrain.py` but with three substantive
differences: **masked loss**, **dynamic-padded batches**, and a **fresh
optimiser** (we don't resume pretrain's Adam state).

#### §1 `FTConfig`

SFT-appropriate defaults, tuned for the 2048-context / 97M model on 8 GB:

| Knob | Value | Why |
|---|---|---|
| `max_len` (documentary) | 2048 | The real `max_len` is loaded from the pretrain checkpoint's `model_cfg`; this field just self-declares what we're tuned for |
| `epochs` | **2** | 3 519 train examples → overfitting risk; bump back to 3 only if val_loss is still falling at end of epoch 2 |
| `micro_batch_size` | 2 | Fits at 2048 context without gradient checkpointing. Drop to 1 if OOM |
| `grad_accum_steps` | 16 | Effective batch = 2 · 16 = **32** |
| `gradient_checkpointing` | False | Off by default; first OOM fallback |
| `max_lr → min_lr` | 2e-5 → 2e-6 cosine | One-to-two orders below pretrain (`3e-4`) — pretrained weights are already in a good basin |
| `warmup_ratio` | 0.03 | 3 % of total steps; Pre-LN doesn't need much warmup |
| `weight_decay` | 0.1, selective | Same LLaMA recipe as pretrain (norms / biases / embeddings exempt) |

#### §2 `FinetuneDataset` + `make_collate`

`FinetuneDataset` loads `train.jsonl` / `val.jsonl` into RAM at startup —
SFT corpora are small (a few MB after tokenization), no point memmapping.

`make_collate(pad_idx)` returns a closure that:
...
2. Builds the **labels tensor** by shifting `input_ids` by 1 and gating
   ...
   Pad positions → `-100`.
3. Builds `attn_keep` (1 = real token, 0 = pad). Currently unused by
   the model — the causal mask + `ignore_index=-100` is sufficient — but
   kept for future KV-cached inference where you'd want to truncate at
   real length.

**The shift-by-1 detail**: at training position `t`, the model sees
`input_ids[t]` and must predict `input_ids[t+1]`. So `labels[t] =
input_ids[t+1]` *if* `loss_mask[t+1] == 1`, else `IGNORE`. The very last
position has no next-token to predict, so its label stays `IGNORE`.

#### §3 `masked_lm_loss`

Plain `F.cross_entropy` with `ignore_index=-100`. Mean is over the
non-ignored positions only — each response token contributes equally
regardless of how much prompt preceded it.

#### §4 `get_lr(step, total_steps, warmup_steps, max_lr, min_lr)`
...
#### §6 `load_pretrained(ckpt_path, gradient_checkpointing, device)`
...
3. Build a fresh `GPTv3`, load `model_state`. **Do not** load
   `optim_state` — fine-tune starts a new cosine schedule from a clean
   optimiser.
...
Token-weighted average loss over the val loader. Multiplies per-batch
loss by the per-batch non-ignored token count, then divides by total
non-ignored tokens at the end. Avoids the "mean-over-batches" trap that
would over-weight short batches.

#### §8 `generate_sample`

Builds the same `<problem>...</problem>\n<reasoning>\n` prefix used at
training time and calls `model.generate(...)` with `temperature=0.7,
top_k=40`. Tag constants and special-token IDs are imported lazily from
[finetune_data_prep.py](finetune_data_prep.py) (`PROBLEM_OPEN`,
`PROBLEM_CLOSE`, `REASON_OPEN`, `BOS_IDX`, `EOS_IDX`) so the two files
can't drift.

Logged every `sample_interval=400` steps. If the masked region is wrong
in `verify_finetune_data.py`, the generated samples will be visibly
broken before any quantitative metric is.
...
**Subtle bit at epoch end**: if the loader runs out before filling the
accum buffer, the remaining micro-batches are still flushed as one
final optimiser step, with the loss scale adjusted to `1/len(buffer)`
instead of `1/grad_accum_steps`. Otherwise the last micro-batches per
epoch would silently never reach the optimiser.
...
- `--epochs N`, `--micro_batch N`, `--grad_accum N`, `--lr F` — quick
  knob tweaks.
- `--gradient_checkpointing` — enable if SFT OOMs at 2048 context.
- `--resume` — restart from FT `last.pt`.

---

### Running the full SFT pipeline

```bash
# 0. CPU-only — safe to run while pretrain is still training:
cd phase3
python finetune_data_prep.py                 # → finetune_data_v2/
python verify_finetune_data.py               # sanity-check mask + previews

# 1. Once pretrain has a usable best.pt:
python finetune.py
python finetune.py --resume                  # from FT last.pt
python finetune.py --gradient_checkpointing  # OOM fallback
python finetune.py --epochs 3 --lr 1e-5      # gentler schedule
python finetune.py --pretrain checkpoints_pretrain_v2/last.pt   # use last instead of best
```

Checkpoints → `checkpoints_finetune_v2/{last,best}.pt`. Training log →
`checkpoints_finetune_v2/log.jsonl` (same JSONL format as pretrain).
...
- Initial val loss should sit somewhere in the 1.5–3.0 range (pretrain
  ...
  hasn't seen, so the first step is naturally higher).
...
- Watch the generated samples after step ~400: should produce
  ...
  doesn't yet solve the problem. If you see only repeated tokens or
  pure prose, the masking is probably wrong (e.g. `loss_mask` inverted)
  — run `verify_finetune_data.py` again to confirm.
- With only 2 epochs and 3 519 examples, train and val should remain
  close. If train collapses to near-zero while val rises, you've
  overfit and should stop early — `best.pt` is what you want.

### Known gotchas

- **Tokenizer drift**: `finetune.py` reads `tokenizer_path` from
  `finetune_data_v2/meta.json`, which is the **path as written at data
  prep time**. If you move directories, regenerate the meta file or
  pass `--data` to point at a freshly re-prepped corpus.
- **Loss mask shift**: the boundary is on the *shifted* mask, not the
  ...
  if you modify `make_collate`.
- **`generate_sample` import**: the tag constants are imported from
