...
```
These are *added* to the token embeddings (which is why embeddings get scaled by √d_model — see [[Weight tying]]).

## Connects to
Superseded in [[Phase 2 - LLaMA-class decoder]] by [[RoPE]], which encodes *relative* position by rotating Q/K instead of adding to the input.
