...
- **[[Gradient checkpointing]]** — recompute activation in backward instead of storing them; ~30 % more compute for a large memory saving.
- **[[Gradient accumulation]]** — simulate a big effective batch by summing grads over micro-batches.
- **[[FlashAttention]]** — O(N) attention memory instead of O(N²); never materializes the score matrix.
- **[[KV-cache]]** — makes generation/rollout cheap at inference time.
...
Full-parameter GRPO on a 7B model will *not* fit 8 GB. Plan for a ≤1.5B policy, LoRA / quantization, gradient checkpointing, tiny group sizes, and a fast rollout backend (e.g. vLLM). Even the open-r1 team faces OOM scaling a 32B SFT past 20k context on multi-node H100s.
